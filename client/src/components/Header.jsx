    ...
    <header className={`sticky top-0 z-50 shadow-lg border-b border-rolex-gold/30 transition-all duration-300 ${scrolled ? 'backdrop-blur-xl "bg-rolex-darkGreen' : 'bg-rolex-darkGreen'}`}>
      ...
      <div className="container mx-auto px-4 py-4">
          ...
          {isAdminRoute ? (
            <div className="flex items-center space-x-3">
              <div className="w-12 h-12 rounded-xl flex items-center justify-center shadow-lg">
                <img src="/logo.png" alt="S & C Tours" />
              </div>
              <h1 className={`text-2xl md:text-3xl font-cinzel font-bold tracking-tight ${scrolled ? 'text-white drop-shadow-lg' : 'text-rolex-gold'}`}>Admin</h1>
            </div>
          ) : (
        ...
        {!isAdminRoute && isMenuOpen && (
          <div className="md:hidden mt-4 pb-4 border-t border-rolex-gold/30 pt-4 animate-slide-down bg-rolex-darkGreen rounded-xl shadow-lg">
