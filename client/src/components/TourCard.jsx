...
const TourCard = ({ tour }) => {
  return (
    <div className="group bg-rolex-emeraldGray rounded-3xl shadow-xl overflow-hidden border-2 border-rolex-gold transition-all duration-300 hover:shadow-2xl hover:-translate-y-2 hover:scale-[1.03] cursor-pointer relative">
      {/* Image */}
      <div className="relative h-56 md:h-64 w-full overflow-hidden">
        <img
          src={tour.image}
          alt={tour.title}
          className="w-full h-full object-cover object-center border-b-2 border-rolex-gold transition-transform duration-500 group-hover:scale-110"
          draggable={false}
        />
        {/* Price Badge */}
        {tour.price && (
      ...
      <div className="p-6 flex flex-col gap-2">
        <h3 className="text-2xl font-cinzel font-bold text-rolex-gold mb-1 line-clamp-2 drop-shadow">
          {tour.title}
        </h3>
        ...
        <div className="flex items-center gap-2 text-rolex-champagne mb-2">
          <Clock className="w-5 h-5" />
          <span className="text-base">{tour.duration}</span>
        </div>
        <p className="text-rolex-champagne mb-4 line-clamp-2">
