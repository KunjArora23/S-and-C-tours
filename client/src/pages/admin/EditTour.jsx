...
const EditTour = () => {
  ...
  useEffect(() => {
    const fetchTour = async () => {
      try {
        const res = await axios.get(`${import.meta.env.VITE_BACKEND_URL}/api/v1/tour/updatetour/${id}`);
        setTour({
  ...
  const submitWithPin = async (pin) => {
    ...
    try {
      if (pendingAction === 'update') {
        await axios.put(`${import.meta.env.VITE_BACKEND_URL}/api/v1/admin/updatetour/${id}`, { ...form, pin }, {
          headers: { "Content-Type": "multipart/form-data" },
          withCredentials: true,
        });
      } else if (pendingAction === 'delete') {
        await axios.delete(`${import.meta.env.VITE_BACKEND_URL}/api/v1/admin/updatetour/${id}`, { data: { pin } });
      }
