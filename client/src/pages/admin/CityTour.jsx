...
const CityTours = () => {
  ...
  const fetchTours = async () => {
    try {
      const res = await axios.get(`${import.meta.env.VITE_BACKEND_URL}/api/v1/admin/get-tours/city/${cityId}`, {
  ...
  const handleDelete = async (tourId) => {
    ...
    try {
      await axios.delete(`${import.meta.env.VITE_BACKEND_URL}/api/v1/admin/delete-tour/${tourId}`, {
        withCredentials: true,
      });
  ...
  const saveNewOrder = async (updatedTours) => {
    try {
      ...
      await axios.post(`${import.meta.env.VITE_BACKEND_URL}/api/v1/tour/admin/reorder`, {
        tourOrders
      });
  ...
  return (
    <div className="container mx-auto px-4 py-8">
      ...
      {loading ? (
        <div className="flex flex-col justify-center items-center h-64 bg-rolex-champagne">
          <div className="animate-spin rounded-full h-12 w-12 border-t-2 border-b-2 border-rolex-gold mb-4"></div>
          <span className="text-lg font-cinzel font-bold text-rolex-gold">Loading tours...</span>
        </div>
      ) : error ? (
