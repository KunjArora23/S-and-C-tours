...
const ReviewManagement = () => {
  ...
  const fetchReviews = async () => {
    try {
      setLoading(true);
      const response = await axios.get('http://sandctour.duckdns.org/api/v1/review/');
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (editingReview) {
        await axios.put(`http://sandctour.duckdns.org/api/v1/review/${editingReview._id}`, formDataToSend, {
          withCredentials: true
        });
        toast.success('Review updated successfully');
      } else {
        await axios.post('http://sandctour.duckdns.org/api/v1/review/create', formDataToSend, {
          withCredentials: true
        });
  ...
  const submitWithPin = async (pin) => {
    ...
    try {
      if (pendingAction === 'update') {
        await axios.put(`http://sandctour.duckdns.org/api/v1/review/${editingReview._id}`, { ...formData, pin }, {
          withCredentials: true
        });
        toast.success('Review updated successfully');
      } else if (pendingAction === 'delete') {
        await axios.delete(`http://sandctour.duckdns.org/api/v1/review/${editingReview._id}`, {
          data: { pin },
          withCredentials: true
        });
  ...
  const handleToggleStatus = async (reviewId) => {
    try {
      await axios.patch(`http://sandctour.duckdns.org/api/v1/review/${reviewId}/toggle`, {}, {
        withCredentials: true
      });
  ...
  const saveNewOrder = async (updatedReviews) => {
    try {
      ...
      await axios.post('http://sandctour.duckdns.org/api/v1/review/admin/reorder', {
        reviewOrders
      });
