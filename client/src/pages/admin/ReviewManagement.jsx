  ...
  const submitWithPin = async (pin) => {
    ...
    try {
      if (pendingAction === 'update') {
        await axios.put(`http://localhost:8000/api/v1/review/${editingReview._id}`, { ...formData, pin }, {
          withCredentials: true
        });
        toast.success('Review updated successfully');
      } else if (pendingAction === 'delete') {
        await axios.delete(`http://localhost:8000/api/v1/review/${editingReview._id}`, {
          data: { pin },
          withCredentials: true
        });
