  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const res = await axios.post(
        'http://sandctour.duckdns.org/api/v1/admin/createcity', 
        formData, 
        {
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <div className="flex justify-end gap-3 pt-2">
          ...
          <button
            type="submit"
            disabled={loading}
            className={`px-6 py-2 rounded-md font-medium border-2 border-rolex-gold ${
              loading 
                ? 'bg-rolex-gold/60 text-rolex-green cursor-not-allowed' 
                : 'bg-rolex-gold hover:bg-rolex-brassHover text-rolex-green'
            } transition-colors`}
          >
            {loading ? (
              <span className="flex items-center justify-center">
                <svg className="animate-spin -ml-1 mr-2 h-4 w-4 text-rolex-gold" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
