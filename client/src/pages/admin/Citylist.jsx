...
const CityList = () => {
  ...
  const fetchCities = async () => {
    try {
      setLoading(true);
      const res = await axios.get("http://sandctour.duckdns.org/api/v1/city/getAll");
  ...
  const handleDelete = async (e, cityId) => {
    ...
    try {
      await axios.delete(`http://sandctour.duckdns.org/api/v1/admin/delete-city/${cityId}`, { withCredentials: true });
  ...
  const saveNewOrder = async (updatedCities) => {
    try {
      ...
      await axios.post('http://sandctour.duckdns.org/api/v1/city/admin/reorder', {
        cityOrders
      });
    ...
    <div className="container mx-auto px-4 py-8">
      ...
      {loading ? (
        <div className="flex flex-col justify-center items-center h-64 bg-rolex-champagne">
          ...
          <span className="text-lg font-cinzel font-bold text-rolex-gold">Loading cities...</span>
        </div>
      ) : error ? (
